...
def __insertRec(dictTree,word,index):
...
def __searchRec(dictTree,word,index):
    current = dictTree #no atual
    if index == len(word): #se chegar no final da palavra
...
def __deleteRec(dictTree,word,index):
    current = dictTree #no atual
...
def showAllWords(): #mostra as palavras da arvore
    print(__generateArray(__trie,"",[]))
    return 

def __generateArray(dictTree,currentString,words):
    current = dictTree #no atual
...
def __insertPrefix(prefix,word):
    return prefix+word

def __iterateList(words,prefix):
    res = []
    for word in words:
...
def __getSubTreeAt(dictTree,word,index):
    current = dictTree #no atual
...
def random(): #retorna uma palavra aleatoria
    return __randomRec(__trie)

def __randomRec(dictTree):
    current = dictTree  #no atual
...
def __debuggingDict(): # função de debugging que mostra o esquema da arvore formatada
